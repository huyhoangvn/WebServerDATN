...
const addHoaDonVaMonDat = async (req, res, next) => {
    ...
    try {
        ...
        console.error(error);
        res.json({ message: "Lỗi khi thêm mới hóa đơn và món đặt", success: false });
    }
}
...
const getDanhSachMonDatByIdHoaDon = async (req, res, next) => {
    try {
        ...
        console.error(error);
        return ({ msg: 'Đã xảy ra lỗi khi lấy danh sách hóa đơn của món đặt', error: error.message, success: false });
    }
};
